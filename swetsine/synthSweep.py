...
postfade = np.ceil( min( N/10.0, max( FS/f2, N/200.0) ) )
##
...
## mag = [sqrt(f1./f(1:end))];
## mag(1) = mag(2);
mag = np.sqrt( f1 /f)           # esto da un warning pero el resultado es igual que
                                # la expresion de Matlab, el primer bin es 'inf'
mag[0] = mag[1]                 # apaño para el 'inf'
##
...
W1, H1 = signal.freqz(B1, A1, worN=(N+1))   # (i) freqz en Scipy no usa la FS
W2, H2 = signal.freqz(B2, A2, worN=(N+1))
W1 = W1 / (2*np.pi) * FS                    # frecs normalizadas --> físicas
W2 = W2 / (2*np.pi) * FS
##
